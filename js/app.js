...
function renderBattle(){
  ...
  if(state.hiddenBossMode || state.gameMode === "hidden" || q.hiddenBoss || state.subject === "Newton Gate"){
    $("questionMeta").textContent=`∂ 隱藏題 ${state.hiddenBossIndex}/${HIDDEN_CALCULUS_QUESTIONS.length}｜Newton Gate｜連對 ${state.correctStreak}｜連錯 ${state.wrongStreak}`;
  } else {
    const bossLabel = state.bossMode ? "👑 Boss 題｜" : "";
  ...
  updateSidebar();
  renderBuffHud();
  const itemDrawer = $("battleItemDrawer");
  if(itemDrawer) itemDrawer.style.display = (state.hiddenBossMode || state.gameMode === "hidden" || state.subject === "Newton Gate") ? "none" : "";
}

function catBattleLine(){
  if(state.hiddenBossMode) return "Pixel 壓低耳朵：喵……Newton Gate 已啟動，請專注作答。";
...
function showFeedback(ok,q,choice,novaText){
  const isHiddenNewtonBoss = !!(state.hiddenBossMode || state.gameMode === "hidden" || q.hiddenBoss || q.subject === "Newton Gate");
  window.aiAssistantPayload = {
    question: q.question || q.Question || "",
    context: q.context || q.Context || q.passage || q.Passage || "",
    ...
    explanation: q.explanation || q.Explanation || q.Solution_Steps || "",
    hint: isHiddenNewtonBoss ? "" : (q.hint || q.Hint || ""),
    subject: q.subject || q.Subject || q.Subject_Area || state.subject || "",
    topic: q.topic || q.Topic_Name || q.type || q.Question_Type || "",
    difficulty: q.difficulty || q.Difficulty || state.difficulty || "",
    isCorrect: !!ok,
  };
  ...
  const aiActionButtonHtml = isHiddenNewtonBoss ? "" : `<button class="secondary-btn" onclick="requestAIAssistance()">📝 本地錯題整理</button>`;
  const localAiHtml = isHiddenNewtonBoss ? "" : `
    <div id="aiAssistantArea" class="ai-assistant-card local-ai-card local-ai-main">
      <div class="ai-assistant-title">本地錯題整理</div>
      <div class="local-ai-subtitle">主要學習區，不需要 API key。先看這裡的重點、錯因與複習提醒，再決定要不要追問外部 AI。</div>
      <div id="aiAssistantContent" class="ai-assistant-content">按一下「本地錯題整理」，會根據本題題目、選項、詳解與提示產生學習建議。此功能不需要 API key。</div>
    </div>`;
  const pickaxePreview = isHiddenNewtonBoss ? "" : escapeHtml(buildAssistPrompt(window.aiAssistantPayload, "explain"));
  const pickaxeTutorHtml = isHiddenNewtonBoss ? "" : `
    <section id="pickaxeInlineTutor" class="pickaxe-inline-card pickaxe-aux-card" aria-label="Pickaxe AI 助教輔助追問">
      <div class="pickaxe-inline-head">
        <div>
          <div class="pickaxe-inline-title">外部 AI 追問（輔助）</div>
          <div class="pickaxe-inline-subtitle">看完上方本地整理後，可選一個追問。系統會自動整理並填入下方 Pickaxe 輸入框；若 Pickaxe 原本的送出鍵被長文字擠掉，可直接按「送出到 AI」。</div>
        </div>
      </div>
      <div class="pickaxe-template-grid" aria-label="推薦追問選項">
        <button class="pickaxe-template-btn active" type="button" data-pickaxe-template="explain" onclick="fillPickaxePrompt('explain', true)">白話解釋這題</button>
        <button class="pickaxe-template-btn" type="button" data-pickaxe-template="whyWrong" onclick="fillPickaxePrompt('whyWrong', true)">我為什麼錯？</button>
        <button class="pickaxe-template-btn" type="button" data-pickaxe-template="notes" onclick="fillPickaxePrompt('notes', true)">整理成錯題筆記</button>
        <button class="pickaxe-template-btn" type="button" data-pickaxe-template="practice" onclick="fillPickaxePrompt('practice', true)">給我類似題</button>
      </div>
      <div class="pickaxe-query-box">
        <textarea
          id="pickaxePromptLine"
          class="pickaxe-query-input pickaxe-query-hidden"
          readonly
          aria-label="已整理好的 AI 提問內容，畫面中已隱藏"
        >${pickaxePreview}</textarea>
        <div class="pickaxe-copy-status" id="pickaxeCopyStatus">已準備好本題資料。點選上方追問選項後會自動填入，下方若看不到 Pickaxe 的送出鍵，可按右側「送出到 AI」。</div>
        <div class="pickaxe-query-actions">
          <button class="pickaxe-send-btn" type="button" onclick="submitPickaxePrompt()">送出到 AI</button>
          <button class="pickaxe-secondary-btn" type="button" onclick="copyPickaxePromptFromBox()">手動複製目前追問</button>
        </div>
      </div>
      <div class="pickaxe-inline-tip" id="pickaxeInlineTip">提示：這裡使用 Pickaxe Chat Input。推薦追問會自動填入；送出請優先使用「送出到 AI」，避免 Pickaxe 內建送出鍵被長文字擠掉。</div>
      <div class="pickaxe-script-shell" id="pickaxeScriptShell">
        <div id="deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df" class="pickaxe-script-deployment"></div>
      </div>
    </section>`;
  $("feedbackArea").innerHTML=`
  ...
  if(!isHiddenNewtonBoss){
    ensurePickaxeScriptEmbed();
...
function toggleAIAssistanceMode(){
  ...
  if(target){
    target.innerHTML = state.aiAssistEnabled
      ? "助教自動模式已開啟，答題後會自動整理本題。"
      : "助教自動模式已關閉，按一下 AI 助教整理可手動查看。";
  }
...
function requestAIAssistance(){
  ...
  if(!window.aiAssistantPayload){
    if(target) target.innerHTML = "無可用題目資料，請完成作答後再查看助教整理。";
    return;
  }
  if(!target) return;
  target.innerHTML = buildLocalAIAssistanceHtml(window.aiAssistantPayload);
}
...
function getChoiceLetterFromList(choices, value){
  const normalizedValue = normalizeAssistText(value);
  const index = (choices || []).findIndex(c => normalizeAssistText(c) === normalizedValue || String(c).trim() === String(value).trim());
  return index >= 0 ? "ABCD"[index] : "";
}

function formatAssistChoice(value, choices){
  const letter = getChoiceLetterFromList(choices, value);
  const text = normalizeAssistText(value);
  return `${letter ? letter + ". " : ""}${text || value || "未作答"}`;
}

function pickSubjectStrategy(subject, topic){
  const text = `${subject || ""} ${topic || ""}`;
  if(/數|math|代數|幾何|函數|比例|統計|機率|圖表|座標/i.test(text)){
    return "先把題目給的條件改寫成式子或圖形，再檢查單位、比例、正負號與題目問的是哪一個量。";
  }
  if(/英|english|reading|dialogue|grammar|cloze|vocabulary/i.test(text)){
    return "先抓句子或短文的主旨，再看轉折詞、代名詞指涉與時態；不要只用單一單字猜答案。";
  }
  if(/國|chinese|閱讀|文意|修辭|成語|古文|語文/i.test(text)){
    return "先判斷題目問字義、文意、修辭還是推論，再回到上下文找能支持答案的句子。";
  }
  if(/自然|science|物理|化學|生物|地科|實驗|圖表/i.test(text)){
    return "先分辨題目是在問觀察、變因、因果或圖表趨勢，再用資料直接支持答案，避免憑印象作答。";
  }
  if(/社會|social|歷史|地理|公民|法規|經濟|人口/i.test(text)){
    return "先找時間、地點、角色、制度或資料來源，再把選項逐條對照，不要把常識印象直接套進題目。";
  }
  return "先找題幹的限制條件，再逐一檢查選項是否完全符合；不要只看關鍵字或第一眼感覺。";
}

function extractAssistTokens(text){
  const s = normalizeAssistText(text);
  const matches = s.match(/[A-Za-z]+|\d+(?:\.\d+)?%?|[一-鿿]{2,}/g) || [];
  const stop = new Set(["因為","所以","但是","如果","可以","不能","不是","以及","沒有","一個","這個","那個","學生","老師","題目","答案","選項","which","what","when","where","that","this","with","from","the","and","but","not"]);
  return [...new Set(matches.map(x => x.trim()).filter(x => x && !stop.has(x.toLowerCase())))].slice(0, 8);
}

function buildDifferenceNote(selected, answer){
  const sTokens = extractAssistTokens(selected);
  const aTokens = extractAssistTokens(answer);
  const onlyA = aTokens.filter(t => !sTokens.includes(t)).slice(0, 4);
  const onlyS = sTokens.filter(t => !aTokens.includes(t)).slice(0, 4);
  if(onlyA.length || onlyS.length){
    const parts = [];
    if(onlyA.length) parts.push(`正解比較強調「${onlyA.join("、")}」`);
    if(onlyS.length) parts.push(`你的選項多了或偏向「${onlyS.join("、")}」`);
    return parts.join("；") + "。";
  }
  return "兩個選項看起來接近時，要回到題幹逐字比對：是否多了絕對化說法、因果方向是否相反、範圍是否被放大或縮小。";
}

function pickaxeTaskInstruction(mode){
  switch(mode){
    case "whyWrong":
      return "我想問：我為什麼會選錯？請先安慰我，再指出我可能誤會的觀念，並用選項比較說明正解為什麼比較好。";
    case "notes":
      return "我想問：請幫我整理成錯題筆記。格式請包含：本題考點、我錯在哪裡、正確解法、易錯提醒、下次判斷口訣。";
    case "practice":
      return "我想問：請先簡短解釋這題，然後出一題同觀念、難度接近的類似題給我練習。先不要公布類似題答案，等我回答後再批改。";
    case "hint":
      return "我想問：請只給我提示，不要直接公布答案。請一步一步引導我自己想出來。";
    case "explain":
    default:
      return "我想問：請用國中生聽得懂的白話方式，一步一步解釋這題。請說明這題考什麼、為什麼正解是這個、下次遇到類似題目怎麼判斷。";
  }
}

function buildAssistPrompt(payload, mode="explain"){
  const choicesText = (payload.choices || []).map((c, i) => `${"ABCD"[i] || i+1}. ${normalizeAssistText(c)}`).join("\n");
  return [
    "請你扮演國中會考 AI 助教，用繁體中文、純文字、白話方式教我這題。",
    "請不要使用 Markdown 表格、特殊符號或太複雜的格式。",
    payload.subject ? `科目：${payload.subject}` : "",
    payload.topic ? `題型：${payload.topic}` : "",
    payload.context ? `材料：${payload.context}` : "",
    `題目：${payload.question || ""}`,
    `選項：\n${choicesText}`,
    `我選：${formatAssistChoice(payload.choice, payload.choices)}`,
    `正解：${formatAssistChoice(payload.answer, payload.choices)}`,
    payload.explanation ? `原解析：${payload.explanation}` : "",
    payload.hint ? `提示：${payload.hint}` : "",
    pickaxeTaskInstruction(mode)
  ].filter(Boolean).join("\n");
}
...
function writeTextToClipboard(text, successMessage){
  if(navigator.clipboard && navigator.clipboard.writeText){
    return navigator.clipboard.writeText(text).then(()=>{
      if(successMessage) setCatMessage("AI 助教", successMessage);
      return true;
    }).catch(()=>{
      window.prompt("請手動複製這段文字：", text);
      return false;
    });
  }
  window.prompt("請手動複製這段文字：", text);
...
function updatePickaxeTemplateActive(mode){
  document.querySelectorAll(".pickaxe-template-btn").forEach(btn=>{
    btn.classList.toggle("active", btn.dataset.pickaxeTemplate === mode);
  });
}

function ensurePickaxeScriptEmbed(){
  const shell = $("pickaxeScriptShell");
  const deployment = $("deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df");
  if(!shell || !deployment) return false;

  // 安全版：不長時間監聽 Pickaxe 內部 DOM，避免 MutationObserver 因 style 變更反覆觸發造成卡住。
  // 這個 deployment div 是答題後動態產生，因此每次答題後只重載一次 Pickaxe bundle。
  if(deployment.dataset.pickaxeReady === "1" || deployment.childElementCount > 0){
    schedulePickaxeLayoutEnhancements();
    return true;
  }

  const oldScript = document.querySelector('script[data-pickaxe-embed="chat-input"]');
  ...
  script.dataset.pickaxeEmbed = "chat-input";
  script.onload = () => {
    deployment.dataset.pickaxeReady = "1";
    schedulePickaxeLayoutEnhancements();
    const status = $("pickaxeCopyStatus");
    if(status && !status.dataset.userAction){
      status.textContent = "Pickaxe Chat Input 已載入。點選上方追問選項後，會嘗試自動填入輸入框。";
    }
  };
  script.onerror = () => {
    updatePickaxeActionStatus("Pickaxe 載入失敗。你仍可用手動複製目前追問。", "warn");
  };
...
function visiblePickaxeField(el){
  if(!el) return false;
  if(el.id === "pickaxePromptLine") return false;
...
function setTextLikeUser(el, text){
  if(!el) return false;
  try{
  ...
  try{
    if(el.isContentEditable){
      ...
      autoGrowPickaxeField(el);
      return normalizeAssistText(el.textContent).length > 0;
    }
...
function collectTextFields(root, out=[]){
  if(!root) return out;
  const selector = 'textarea, input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), [contenteditable="true"], [role="textbox"]';
  try{
    if(root.matches && root.matches(selector)) out.push(root);
    if(root.querySelectorAll){
      root.querySelectorAll(selector).forEach(el=>out.push(el));
      // 安全限制：避免對 Pickaxe 產生的大量 DOM 反覆全量掃描造成網頁卡住。
      const all = Array.from(root.querySelectorAll("*")).slice(0, 180);
      all.forEach(el=>{
        if(el.shadowRoot && out.length < 20) collectTextFields(el.shadowRoot, out);
      });
    }
  }catch(e){}
...
function autoGrowPickaxeField(el){
  if(!el) return;
  const tag = (el.tagName || "").toLowerCase();
  const isBox = tag === "textarea" || el.isContentEditable || el.getAttribute("role") === "textbox";
  if(!isBox) return;
  try{
    el.style.minHeight = el.style.minHeight || "78px";
    ...
    if(tag === "textarea"){
      ...
      const next = Math.min(Math.max(el.scrollHeight, 78), 230);
      el.style.height = `${next}px`;
    }
  }catch(e){}
}

function enhancePickaxeChatInputLayout(){
  const deployment = $("deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df");
  const shell = $("pickaxeScriptShell");
  if(shell){
  ...
  const roots = [deployment];
  if(deployment.shadowRoot) roots.push(deployment.shadowRoot);

  const fields = [...new Set(roots.flatMap(root => collectTextFields(root, [])))]
    .filter(el => el.id !== "pickaxePromptLine");

  fields.forEach(field => {
    autoGrowPickaxeField(field);
    if(!field.dataset.pickaxeAutoGrowBound){
      field.dataset.pickaxeAutoGrowBound = "1";
      field.addEventListener("input", () => autoGrowPickaxeField(field));
      field.addEventListener("change", () => autoGrowPickaxeField(field));
    }

    const row = field.closest ? field.closest("form, div") : null;
    if(row){
      try{
        row.style.maxWidth = "100%";
        row.style.boxSizing = "border-box";
        if(row.querySelector && row.querySelector("button, [role='button']")){
  ...
  roots.forEach(root => {
    try{
      if(root.querySelectorAll) root.querySelectorAll("button, [role='button']").forEach(btn => buttons.push(btn));
    }catch(e){}
  });
  buttons.forEach(btn => {
    try{
      ...
      btn.style.minWidth = btn.style.minWidth || "48px";
...
function startPickaxeLayoutObserver(){
  // 舊名稱保留給其他函式呼叫；實際改為短時間排程，不再持續監聽。
  schedulePickaxeLayoutEnhancements();
}

function schedulePickaxeLayoutEnhancements(){
  const deployment = $("deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df");
  if(!deployment) return;
  enhancePickaxeChatInputLayout();
  if(deployment.dataset.pickaxeLayoutScheduled === "1") return;
  deployment.dataset.pickaxeLayoutScheduled = "1";
  [200, 600, 1200, 2200, 3600].forEach(delay => {
    setTimeout(() => {
      enhancePickaxeChatInputLayout();
      if(delay === 3600 && deployment){
        delete deployment.dataset.pickaxeLayoutScheduled;
      }
    }, delay);
  });
}

function scorePickaxeField(el){
  let score = 0;
  const tag = (el.tagName || "").toLowerCase();
  const hint = `${el.getAttribute("placeholder") || ""} ${el.getAttribute("aria-label") || ""} ${el.getAttribute("name") || ""} ${el.getAttribute("role") || ""}`.toLowerCase();
  if(tag === "textarea") score += 40;
  if(el.isContentEditable || el.getAttribute("role") === "textbox") score += 35;
  if(/ask|chat|message|type|輸入|提問|問題|回答|訊息|助教/.test(hint)) score += 30;
  const rect = el.getBoundingClientRect ? el.getBoundingClientRect() : { width:0, height:0, top:0 };
  score += Math.min(20, Math.round(rect.width / 50));
  score += Math.min(15, Math.round(rect.height / 8));
  score += Math.max(0, Math.round(rect.top / 200));
  return score;
}

function tryFillPickaxeInput(prompt){
  const deployment = $("deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df");
  const panel = $("pickaxeInlineTutor");
  if(!deployment) return { ok:false, reason:"找不到 Pickaxe Chat Input 區塊" };

  const frame = deployment.querySelector("iframe") || (deployment.shadowRoot && deployment.shadowRoot.querySelector("iframe"));
  if(frame){
    return { ok:false, reason:"Pickaxe 仍把 Chat Input 包在 iframe 中，外層網頁無法直接寫入" };
  }
  ...
  const fields = [...new Set(roots.flatMap(root => collectTextFields(root, [])))]
    .filter(visiblePickaxeField)
    .filter(el => el.id !== "pickaxePromptLine");

  if(!fields.length){
    return { ok:false, reason:"尚未偵測到 Pickaxe 產生的可填入輸入框" };
  }

  const candidates = fields
    .map(el => ({ el, score: scorePickaxeField(el) }))
    .sort((a,b) => b.score - a.score);

  for(const item of candidates){
    if(setTextLikeUser(item.el, prompt)){
      autoGrowPickaxeField(item.el);
      enhancePickaxeChatInputLayout();
      try{ item.el.focus(); }catch(e){}
      return { ok:true, field:item.el };
    }
  }
  return { ok:false, reason:"找到輸入框，但無法成功寫入文字" };
}


function collectPickaxeButtons(root, out=[]){
  if(!root) return out;
  ...
  try{
    ...
    if(root.querySelectorAll){
      ...
      const all = Array.from(root.querySelectorAll("*")).slice(0, 180);
      all.forEach(el=>{
        if(el.shadowRoot && out.length < 40) collectPickaxeButtons(el.shadowRoot, out);
      });
    }
  }catch(e){}
...
function getPickaxeSearchRoots(){
  const deployment = $("deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df");
  if(!deployment) return [];
  const roots = [deployment];
  if(deployment.shadowRoot) roots.push(deployment.shadowRoot);
...
function findBestPickaxeInputField(){
  const roots = getPickaxeSearchRoots();
  const fields = [...new Set(roots.flatMap(root => collectTextFields(root, [])))]
    .filter(visiblePickaxeField)
    .filter(el => el.id !== "pickaxePromptLine")
    .map(el => ({ el, score: scorePickaxeField(el) }))
    .sort((a,b) => b.score - a.score);
  return fields.length ? fields[0].el : null;
}

function isVisiblePickaxeButton(btn){
  if(!btn) return false;
...
function scorePickaxeSendButton(btn, field){
  const text = `${btn.textContent || ""} ${btn.value || ""} ${btn.getAttribute("aria-label") || ""} ${btn.getAttribute("title") || ""} ${btn.getAttribute("type") || ""}`.toLowerCase();
  let score = 0;
  if(/send|submit|ask|chat|message|arrow|傳送|送出|提交|提問|詢問|發送/.test(text)) score += 70;
  if(/copy|複製|cancel|close|關閉|clear|清除/.test(text)) score -= 100;
  if(btn.disabled || btn.getAttribute("aria-disabled") === "true") score -= 80;
  const r = btn.getBoundingClientRect ? btn.getBoundingClientRect() : {left:0, top:0, width:0, height:0};
  score += Math.min(20, Math.round((r.width + r.height) / 12));
  if(field && field.getBoundingClientRect){
    const fr = field.getBoundingClientRect();
    const dx = Math.abs((r.left + r.width/2) - (fr.right));
    const dy = Math.abs((r.top + r.height/2) - (fr.top + fr.height/2));
    score += Math.max(0, 40 - Math.round((dx + dy) / 25));
    if(r.left >= fr.left) score += 8;
  }
  return score;
}

function findPickaxeSendButton(field){
  const roots = getPickaxeSearchRoots();
  const buttons = [...new Set(roots.flatMap(root => collectPickaxeButtons(root, [])))]
    .filter(isVisiblePickaxeButton)
    .map(btn => ({ btn, score: scorePickaxeSendButton(btn, field) }))
    .sort((a,b) => b.score - a.score);
  return buttons.length && buttons[0].score > -20 ? buttons[0].btn : null;
}

function clickPickaxeButton(btn){
  if(!btn) return false;
  ...
  try{
    ...
    try{ btn.click(); return true; }catch(_err){ return false; }
  }
}

function submitPickaxePrompt(){
  ensurePickaxeScriptEmbed();
  schedulePickaxeLayoutEnhancements();
  const deployment = $("deployment-1b6c2e72-5695-4e20-a39e-7e6046db53df");
  if(!deployment){
    updatePickaxeActionStatus("找不到 Pickaxe 區塊，請先完成作答後再使用。", "warn");
    return;
  }
  const frame = deployment.querySelector("iframe") || (deployment.shadowRoot && deployment.shadowRoot.querySelector("iframe"));
  if(frame){
    updatePickaxeActionStatus("Pickaxe 目前被包在 iframe 中，外層無法代替你按送出。請手動貼上並送出。", "warn");
    return;
  }

  const field = findBestPickaxeInputField();
  if(!field){
    updatePickaxeActionStatus("尚未找到 Pickaxe 輸入框。請先點上方推薦追問，或等 Pickaxe 載入完成再試一次。", "warn");
    return;
  }

  const currentText = field.isContentEditable ? field.textContent : ("value" in field ? field.value : field.textContent);
  if(!String(currentText || "").trim()){
    const box = $("pickaxePromptLine");
    const prompt = box && box.value ? box.value : (window.aiAssistantPayload ? buildAssistPrompt(window.aiAssistantPayload, "explain") : "");
    if(prompt){
      const filled = tryFillPickaxeInput(prompt);
      if(!filled.ok){
        updatePickaxeActionStatus("Pickaxe 輸入框目前是空的，且無法自動填入。請重新點選推薦追問。", "warn");
        return;
      }
    }
  }

  const button = findPickaxeSendButton(field);
  if(button && clickPickaxeButton(button)){
    updatePickaxeActionStatus("已嘗試按下 Pickaxe 送出鍵。如果沒有送出，請在下方輸入框內按 Enter 或手動點送出。", "success");
    setCatMessage("AI 助教", "已嘗試送出到 Pickaxe。若沒有反應，請手動按 Pickaxe 的送出鍵。");
    return;
  }
  ...
  if(form){
    try{
      ...
      updatePickaxeActionStatus("已嘗試送出 Pickaxe 表單。如果沒有反應，請手動點下方送出鍵。", "success");
      return;
    }catch(e){}
  }
  ...
  try{
    ...
    updatePickaxeActionStatus("找不到可代按的送出鍵，已嘗試用 Enter 送出。若沒有反應，請手動點 Pickaxe 送出鍵。", "warn");
  }catch(e){
    updatePickaxeActionStatus("找不到可代按的送出鍵。請手動點 Pickaxe 內建送出鍵，或使用手動複製。", "warn");
  }
}

function updatePickaxeActionStatus(message, kind="info"){
  const status = $("pickaxeCopyStatus");
  if(status){
    status.dataset.userAction = "1";
...
function putPickaxePromptIntoAssistant(){
  const payload = window.aiAssistantPayload;
  const box = $("pickaxePromptLine");
  const prompt = box && box.value ? box.value : (payload ? buildAssistPrompt(payload, "explain") : "");
  if(!prompt){
    setCatMessage("AI 助教", "目前沒有可填入的提問內容。請先完成一題。");
    return;
  }

  ensurePickaxeScriptEmbed();
  openPickaxeTutor(false);
  startPickaxeLayoutObserver();
  updatePickaxeActionStatus("正在尋找 Pickaxe Chat Input，準備自動填入本題內容。", "info");

  let attempts = 0;
  const maxAttempts = 8;
  const tryNow = () => {
    attempts += 1;
    const result = tryFillPickaxeInput(prompt);
    if(result.ok){
      updatePickaxeActionStatus("已自動填入下方 Pickaxe 輸入框。請按右側「送出到 AI」送出。", "success");
      setCatMessage("AI 助教", "已把本題內容放入 Pickaxe 輸入框，可按「送出到 AI」。");
      return;
    }
    if(attempts < maxAttempts){
      setTimeout(tryNow, 450);
      return;
    }
    writeTextToClipboard(prompt, "Pickaxe 目前無法被直接填入，已改為自動複製。請貼到下方 AI 助教後送出。").finally(()=>{
      updatePickaxeActionStatus(`無法自動填入：${result.reason}。已自動複製，請貼到下方 Pickaxe 輸入框後送出。`, "warn");
    });
  };
  setTimeout(tryNow, 120);
}

function fillPickaxePrompt(mode="explain", copyNow=false){
  const payload = window.aiAssistantPayload;
  if(!payload){
    setCatMessage("AI 助教", "目前沒有題目資料。請先完成一題，再使用推薦追問。");
    return;
  }
  const prompt = buildAssistPrompt(payload, mode);
  const box = $("pickaxePromptLine");
  if(box){
    box.value = prompt;
    box.dataset.promptMode = mode;
  }
  const labelMap = { explain:"白話解釋這題", whyWrong:"我為什麼錯？", notes:"整理成錯題筆記", practice:"給我類似題", hint:"只給提示" };
  updatePickaxeActionStatus(`已選擇「${labelMap[mode] || "白話解釋這題"}」。正在嘗試自動填入下方 AI 輸入框。`, "info");
  updatePickaxeTemplateActive(mode);
  if(copyNow){
    putPickaxePromptIntoAssistant();
  }
}

function copyPickaxePromptFromBox(){
  const payload = window.aiAssistantPayload;
  const box = $("pickaxePromptLine");
  const prompt = box && box.value ? box.value : (payload ? buildAssistPrompt(payload, "explain") : "");
  if(!prompt){
    setCatMessage("AI 助教", "目前沒有可複製的提問內容。請先完成一題。");
    return;
  }
  writeTextToClipboard(prompt, "已手動複製目前追問。請貼到下方 AI 助教輸入框後送出。").finally(()=>{
    openPickaxeTutor();
  });
}

function copyAIAssistPrompt(){
  const payload = window.aiAssistantPayload;
  if(!payload){
    setCatMessage("AI 助教", "目前沒有題目資料。請先完成一題，再複製本題提問。");
    return;
  }
  const prompt = buildAssistPrompt(payload, "explain");
  writeTextToClipboard(prompt, "已複製進階提問，可貼到 Pickaxe 或外部 AI 繼續追問。");
}

function openPickaxeTutor(focusFrame=true){
  const panel = $("pickaxeInlineTutor");
  if(!panel){
    setCatMessage("AI 助教", "外部 AI 追問會在答題後出現在本地錯題整理下方。請先完成一題。");
    return;
  }
  panel.scrollIntoView({ behavior: "smooth", block: "start" });
  panel.classList.add("pickaxe-inline-highlight");
  setTimeout(() => panel.classList.remove("pickaxe-inline-highlight"), 1200);
  const assistantBox = panel.querySelector(".pickaxe-script-shell");
  if(focusFrame && assistantBox) assistantBox.scrollIntoView({ behavior: "smooth", block: "center" });
}

function closePickaxeTutor(){
  // Pickaxe 助教已改為答題後的頁內區塊，不再使用浮動彈窗。
}

function copyAIAssistPromptAndOpenPickaxe(){
  copyPickaxePromptFromBox();
}

function buildLocalAIAssistanceHtml(payload){
  const choices = payload.choices || [];
  const selectedText = formatAssistChoice(payload.choice, choices);
  const answerText = formatAssistChoice(payload.answer, choices);
  const isCorrect = typeof payload.isCorrect === "boolean"
    ? payload.isCorrect
    : normalizeAssistText(payload.choice) === normalizeAssistText(payload.answer);
  const strategy = pickSubjectStrategy(payload.subject, payload.topic);
  const difference = isCorrect ? "你這次選到的重點與正解一致。接下來要練的是把理由說清楚，而不只是記答案。" : buildDifferenceNote(payload.choice, payload.answer);
  const hint = payload.hint || strategy;
  const explanation = payload.explanation || "這題目前沒有提供完整詳解，先用題幹條件與選項差異來判斷。";
  const statusTitle = isCorrect ? "✅ 本題已答對" : "🧩 本題需要訂正";
  const topicLine = [payload.subject, payload.topic, payload.difficulty].filter(Boolean).join("｜") || "一般素養題";
  const reviewTask = isCorrect
    ? `請用一句話說明：為什麼「${answerText}」能符合題幹？`
    : `請用一句話說明：為什麼「${selectedText}」不能取代「${answerText}」？`;
  const wrongNote = isCorrect
    ? `我答對這題，是因為我能抓到「${payload.topic || "題目核心"}」並對照選項細節。`
    : `我這題錯在把「${selectedText}」和正解混淆；下次要先抓題幹限制，再比對「${answerText}」的關鍵條件。`;

  return `
    <div class="ai-assist-summary ${isCorrect ? "is-correct" : "is-wrong"}">
      <div class="ai-assist-badge">${statusTitle}</div>
      <div class="ai-assist-meta">${renderMathText(topicLine)}</div>
    </div>
    <div class="ai-assist-section">
      <div class="ai-assist-heading">1. 這題在考什麼？</div>
      <p>${renderMathText(`題型重點是：${payload.topic || "讀懂題幹並比對選項"}。${hint}`)}</p>
    </div>
    <div class="ai-assist-section">
      <div class="ai-assist-heading">2. 你的答案判斷</div>
      <p>${renderMathText(`你的選擇：${selectedText}`)}</p>
      <p>${renderMathText(`正確答案：${answerText}`)}</p>
      <p>${renderMathText(difference)}</p>
    </div>
    <div class="ai-assist-section">
      <div class="ai-assist-heading">3. 解題流程</div>
      <ol class="ai-assist-steps">
        <li>${renderMathText("先圈出題幹真正要求：是問原因、結果、主旨、計算值，還是最合理推論？")}</li>
        <li>${renderMathText(strategy)}</li>
        <li>${renderMathText("把每個選項放回題幹檢查：有沒有超出範圍、方向相反、只符合一半，或偷換概念？")}</li>
        <li>${renderMathText(`最後對照詳解：${explanation}`)}</li>
      </ol>
    </div>
    <div class="ai-assist-section">
      <div class="ai-assist-heading">4. 錯題筆記 / 回想練習</div>
      <p><strong>可寫進錯題本：</strong>${renderMathText(wrongNote)}</p>
      <p><strong>30 秒回想：</strong>${renderMathText(reviewTask)}</p>
    </div>
    <div class="ai-assist-actions">
      <button class="mini-btn" onclick="copyAIAssistPrompt()">📋 複製進階 AI 提問</button>
      <button class="mini-btn" onclick="copyAIAssistPromptAndOpenPickaxe()">💬 複製並跳到 AI 助教</button>
    </div>
  `;
}
...
// Pickaxe 助教目前為答題後的頁內區塊，不需要鍵盤關閉。
document.addEventListener("keydown", (event)=>{
  if(event.key === "Escape") closePickaxeTutor();
});
