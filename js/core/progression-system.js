...
function useRemoveWrong(){
  ensureItems();
  if(state.hiddenBossMode || state.gameMode === "hidden" || state.subject === "Newton Gate"){ setCatMessage("小喵", "Newton Gate 不開放道具輔助。"); return; }
...
function useHintCard(){
  ...
  const topic = q.topic || q.Topic_Name || q.type || "綜合判斷";
  const hint = q.hint || q.Hint || "先找題幹中的限制條件，不要只看關鍵字。這題的陷阱通常在選項的細節差異。";
  $("feedbackArea").innerHTML = `<div class="feedback-card correct"><div class="feedback-title correct">💡 小喵提示</div><p><strong>題型：</strong>${esc(topic)}</p><p>${esc(hint)}</p><p class="muted">提示卡只給方向，不直接公布答案；作答後可用本地錯題整理複習。</p></div>`;
  updateSidebar();
}
